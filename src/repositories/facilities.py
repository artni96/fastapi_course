...
class RoomsFacilitiesRepository(BaseRepository):
    ...
    async def room_facility_creator(self, room_id: int, new_facility_ids: list[int]):
        current_facility_ids_stmt = select(RoomFacilitiesModel.facility_id).where(
        ...
        facility_ids_to_delete = list(set(current_facility_ids) - set(new_facility_ids))
        facility_ids_to_add = list(set(new_facility_ids) - set(current_facility_ids))
        facility_ids_to_add = [
        ...
        if facility_ids_to_delete:
            query_to_delete = delete(self.model).filter(
                self.model.room_id == room_id,
                self.model.facility_id.in_(facility_ids_to_delete),
            )
            await self.session.execute(query_to_delete)
        if facility_ids_to_add:
            managed_facility_data = [obj.model_dump() for obj in facility_ids_to_add]
            facility_ids_to_add = insert(self.model).values(managed_facility_data)
